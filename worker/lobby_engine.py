...
Architecture (Active Bot):
- Bot creates a room at rooms/{roomCode} as p1 (host)
- Registers room in publicRooms/BOT/{roomCode} for discovery
- Waits for a human player to join as p2
- When p2 joins, bot starts the game (generates turnSecret)
- Plays turn-based game, decides outcome (host responsibility)
- After game ends, cleans up and creates a new room

States: IDLE → LOBBY → PLAYING → FINISHED → (reset) → IDLE
...
_REMATCH_WAIT_SECONDS = 30  # how long to wait for p2's "play again" request


class LobbyState(Enum):
    IDLE = auto()
    LOBBY = auto()      # room created, waiting for p2
    PLAYING = auto()
    FINISHED = auto()
...
class LobbyEngine:
    ...
    def shutdown(self) -> None:
        ...
        self._safe_cleanup()
        # Clean up any remaining room
        if self._room_code:
            try:
                self._root.update({
                    f"rooms/{self._room_code}": None,
                    f"publicRooms/BOT/{self._room_code}": None,
                })
            except Exception:
    ...
    def status_info(self) -> dict:
        """Current state for console monitoring."""
        return {
    ...
    def _do_idle(self) -> None:
        """Create a new room and register in publicRooms."""
        # Reset game state
        self._history = []
        ...
        # Create room + register in publicRooms atomically
        self._root.update({
            # Room data
            ...
            f"rooms/{self._room_code}/groupCode": "BOT",
            ...
            # Public room entry
            f"publicRooms/BOT/{self._room_code}/hostName": self._config.name,
            f"publicRooms/BOT/{self._room_code}/gameMode": "turn",
            f"publicRooms/BOT/{self._room_code}/level": self._config.level,
            f"publicRooms/BOT/{self._room_code}/createdAt": {".sv": "timestamp"},
            f"publicRooms/BOT/{self._room_code}/playerCount": 1,
        })
    ...
    def _do_lobby(self) -> None:
        """Wait for a player to join as p2."""
        # Sync from Firebase first — p2 may have already joined (e.g. rematch)
    ...
    def _check_lobby_transitions(self) -> bool:
        """Check lobby state and transition if needed. Returns True if state changed."""
        ...
        # Check if game was already started (race with iOS maybeStartGameIfReady)
        status = self._room_data.get("status", "lobby")
        if status == "playing":
            turn_secret = self._room_data.get("turnSecret")
            if turn_secret:
                self._turn_secret = turn_secret
                logger.info("→ PLAYING (game started by iOS, room=%s)", self._room_code)
    ...
    def _do_playing(self) -> None:
        """Handle the turn-based game loop as host."""
        try:
            event = self._room_queue.get(timeout=30.0)
        except queue.Empty:
            return
        ...
        # If turnSecret not yet available, game hasn't started
        ...
        # Check if game is over (outcome already set)
        ...
        # Check if p2 solved (host responsibility to detect)
        if self._check_p2_solved():
            return

        # Check if p2 disconnected (node removed OR connected=false)
        players = self._room_data.get("players", {})
    ...
    def _do_finished(self) -> None:
        """Wait for rematch, then clean up room and restart."""
        self._drain_and_sync()
        ...
        if rematch_accepted:
            ...
            # 1. Create new room FIRST (must exist before iOS tries to join)
            self._root.update({
                ...
                f"rooms/{new_code}/groupCode": "BOT",
            ...
            # 2. NOW tell iOS where to go (room already exists)
            room_ref.child("rematch/newRoomCode").set(new_code)

            # 3. Clean up old room listener and switch to new room
            self._safe_cleanup()
            ...
            # 4. Delete old room after delay (iOS needs time to read newRoomCode)
            time.sleep(5)
            try:
                self._root.update({
                    f"rooms/{old_code}": None,
                    f"publicRooms/BOT/{old_code}": None,
                })
            except Exception:
                logger.warning("Failed to delete old room %s", old_code, exc_info=True)
        else:
            # No rematch — clean up and create fresh lobby room
    ...
    def _wait_for_rematch(self) -> bool:
        """Poll for p2's rematch request. Returns True if p2 requested rematch."""
        deadline = time.monotonic() + _REMATCH_WAIT_SECONDS
        while time.monotonic() < deadline:
            if self._shutdown_event.is_set():
                return False
            # Check for events or poll Firebase
            self._drain_and_sync()
            rematch = self._room_data.get("rematch", {})
            if isinstance(rematch, dict) and rematch.get("p2") is True:
                return True
            # Also check if p2 disconnected — no point waiting
            players = self._room_data.get("players", {})
            p2 = players.get("p2") if isinstance(players, dict) else None
            if p2 is None:
    ...
    def _start_game(self) -> None:
        """Start the game as host: generate turnSecret, set status to playing."""
        # Check if iOS already started (race condition guard)
        self._drain_and_sync()
        if self._room_data.get("status") == "playing":
            self._turn_secret = self._room_data.get("turnSecret", "")
            logger.info("Game already started by iOS (race condition handled)")
            return
        ...
        # Remove from publicRooms (room is full)
        self._root.child(f"publicRooms/BOT/{self._room_code}").delete()
    ...
    def _submit_guess(self) -> None:
        ...
        if s == 3:
            ...
            # Host decides outcome immediately
    ...
    def _drain_and_sync(self) -> None:
        """Drain pending events and re-sync room_data from Firebase."""
        while True:
            try:
                self._room_queue.get_nowait()
            except queue.Empty:
    ...
    def _apply_event(self, event: db.Event) -> None:
        ...
        # Update round counter from rounds data
        rounds = self._room_data.get("rounds", {})
    ...
    def _safe_cleanup(self) -> None:
        """Clean up listeners and presence."""
